...
(provide (struct-out abstraction)
         ...
         utility-upper-bound
         arity-zero-best
         ;; INSTRUMENTATION (experiments/2026-08-18-0341Z-search-order):
...
;; INSTRUMENTATION (experiments/2026-08-18-0341Z-search-order).
;; Counters, reset at the top of each `search` call:
;;   pops-expanded  patterns popped off the worklist that survived the cutoff
;;                  re-check and were actually expanded (step!)
;;   pushed         patterns added to the worklist (including the initial one)
;;   considered     calls to consider!
;;   finished       finished (hole-free) patterns scored via `finish`
(define stats-pops-expanded 0)
(define stats-pushed 0)
(define stats-considered 0)
(define stats-finished 0)

;; search-stats : -> (Hash Symbol Natural)
;; The counters from the most recent `search` call.
(define (search-stats)
  (hash 'pops-expanded stats-pops-expanded
        'pushed stats-pushed
...
(define (search c [max-arity 2] #:mode [mode 'best])
  ;; INSTRUMENTATION: reset the counters for this run.
  (set! stats-pops-expanded 0)
  (set! stats-pushed 0)
  (set! stats-considered 0)
  (set! stats-finished 0)

  (define best (arity-zero-best c))
  (define cutoff (if best (abstraction-utility best) 0))

  ;; The worklist.  In 'best mode (the original behavior): a max-heap on the
  ;; upper bound, so the most promising partial pattern is always expanded next
  ;; (compression.rs:598-608).  data/heap pops the minimum, so the comparison
  ;; is reversed.  INSTRUMENTATION adds two alternative orders behind the same
  ;; push!/pop!/empty? interface -- ALL pruning logic is identical across modes,
  ;; only the order of removal differs:
  ;;   'fifo  a plain two-list queue (breadth-ish, approximates the
  ;;          level-synchronous order a Datalog stratum would impose)
  ;;   'lifo  a stack (depth-first)
  (define-values (worklist-push! worklist-pop! worklist-empty?)
    (case mode
      [(best)
       (define h (make-heap (lambda (a b)
                              (>= (pattern-upper-bound a) (pattern-upper-bound b)))))
       (values (lambda (p) (heap-add! h p))
               (lambda () (begin0 (heap-min h) (heap-remove-min! h)))
               (lambda () (zero? (heap-count h))))]
      [(fifo)
       (define front '())
       (define back '())
       (values (lambda (p) (set! back (cons p back)))
               (lambda ()
                 (when (null? front)
                   (set! front (reverse back))
                   (set! back '()))
                 (begin0 (car front) (set! front (cdr front))))
               (lambda () (and (null? front) (null? back))))]
      [(lifo)
       (define stack '())
       (values (lambda (p) (set! stack (cons p stack)))
               (lambda () (begin0 (car stack) (set! stack (cdr stack))))
               (lambda () (null? stack)))]
      [else (error 'search "unknown mode: ~a" mode)]))

  ;; push! : Pattern -> Void  -- add to the worklist, counting it
  (define (push! p)
    (set! stats-pushed (add1 stats-pushed))
    (worklist-push! p))

  (push! (initial-pattern c (lambda (locs) (utility-upper-bound c locs))))
  ...
  (define (consider! p expansion locs)
    (set! stats-considered (add1 stats-considered))   ; INSTRUMENTATION
      ...
      (unless (or (<= bound cutoff)
        ...
        (cond
          [(pattern-finished? child)
           (set! stats-finished (add1 stats-finished))   ; INSTRUMENTATION
