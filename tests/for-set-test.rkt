...
;; Every body mentions its own iteration variable, so a V1 template (a
;; template binder in the inner lambda's position) is H1-blocked at every
;; site -- V2's binder-position pattern variable is not a convenience here,
;; it is the only way any site rewrites at all. The fourth program is the H2
;; exercise: it locally shadows set-add, so the learned macro -- whose
;; set-add is the global -- must refuse that site outright.
;;
...
(module+ test
  (test-case "macro-search recovers for/set"
    ...
    ;; including the H2-shadowed one): about 245 seconds. Comfortably inside
    ;; the ~5-minute budget, so the corpus is kept whole -- no cutting was
