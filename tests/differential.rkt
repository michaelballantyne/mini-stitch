...
;; every corpus in stitch/data/basic and for four configurations of each, it
;; *runs* `stitch/target/release/compress` and compares its `out.json` against
;; what `compress` from src/compress.rkt produces on the same input.  The Rust
;; is ground truth; any difference is a bug in mini-stitch until proven
;; otherwise.
;;
...
(define nuts-bolts (build-path repo-root "stitch" "data" "cogsci" "nuts-bolts.json"))
...
;; pick different bodies of exactly equal utility.  Kept explicit so that a new
;; tie shows up as something to look at rather than as noise.
...
(define (compare-run name max-arity iterations real mini)
    ...
    (for/or ([r (in-list real-abs)] [m (in-list mini-abs)] [i (in-naturals)])
      (cond
        ...
        [(= (hash-ref r 'utility) (step-utility m))
         ...
         (note! "TIE at abstraction ~a (utility ~a): real ~s, mini ~s"
  ...
  (define verdict
    (cond [tie? 'tie]
          [(null? notes) 'match]
          [else 'fail]))
  ;; A tie leaves exactly one note, its own; a failure keeps them all.
  (outcome name max-arity iterations verdict (reverse notes)))
...
(define (print-summary outcomes)
  ...
  (for ([o (in-list outcomes)] #:when (eq? (outcome-verdict o) 'tie))
    (printf "~a tie: ~a at arity ~a, iterations ~a\n"
            (if (member (cons (outcome-corpus o) (outcome-max-arity o)) known-ties)
                "known" "UNEXPECTED")
...
(module+ test
  ...
  (test-case "mini-stitch agrees with the real binary on stitch/data/basic"
    ...
    ;; And one run at realistic scale: 250 programs of cogsci/nuts-bolts.
    (define start (current-inexact-milliseconds))
    (define big (check-corpus "nuts-bolts" nuts-bolts 2 3))
    (define elapsed (- (current-inexact-milliseconds) start))

    (print-summary (append outcomes (list big)))
    (printf "nuts-bolts (250 programs, arity 2, 3 iterations): ~a ms for both systems\n"
            (inexact->exact (round elapsed)))

    (define failures
      (filter (lambda (o) (eq? (outcome-verdict o) 'fail))
              (append outcomes (list big))))
